...
public class ModifyMethodTest {
     ...
     * 
     * @param className
     *            The internal form class name to modify
     * @param methodName
     *            The name of the method to transform
     * @param methodSignature
     *            A regular expression to match the method signature. (if null,
     *            matches ".*")
     * @param classLoader
     *            The intrumentation provided classloader
     * @param byteCode
     *            The pre-transform byte code
     * @return the modified byte code if successful, otherwise returns the
     *         original unmodified byte code
     */
    public static byte[] instrument(String className, String methodName, String methodSignature,
	    ClassLoader classLoader, byte[] byteCode) {
	...
	try {
	    ...
	    Pattern sigPattern = Pattern
		    .compile((methodSignature == null || methodSignature.trim().isEmpty()) ? ".*" : methodSignature);
	    int modifies = 0;
